...
local function banall_by_reply(extra, success, result)
	...
	if is_admin2(result.from.peer_id) then -- Ignore admins
		...
		send_large_msg(channel, "><i> دسترسی کاربر برای ورود به تمامی گروه های ربات </i> [ @TGtelewhite ] <i> مسدود شد </i>\n><i> شناسه کاربری: </i> [<b>"..result.from.peer_id.."</b>]")
	else
		return
	end
end
...
local function unbanall_by_reply(extra, success, result)
	...
	if is_admin2(result.from.peer_id) then -- Ignore admins
     ...
     if not is_gbanned(result.from.peer_id) then
       return '><i> کاربر مورد نظر در لیست مسدودیت دائمی ربات </i> [ @TGtelewhite ] <i> قرار ندارد </i>\n><i> شناسه کاربری: </i> [<b>'..result.from.peer_id..'</b>]'
      end
	  if is_gbanned(result.from.peer_id) then
		...
		send_large_msg(channel, "><i> کاربر مورد نظر در لیست مسدودیت دائمی ربات </i> [ @TGtelewhite ] <i> قرار ندارد </i>\n><i> شناسه کاربری: </i> [<b>"..result.from.peer_id.."</b>]")
	end
	end
end
...
local function kick_ban_res(extra, success, result)
      ...
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, '><i> دسترسی کاربر برای ورود به تمامی گروه های ربات </i> [ @TGtelewhite ] <i> مسدود شد </i>\n><i> یوزرنیم: </i> [@'..member..']\n><i> شناسه کاربری: </i> [<b>'..member_id..'</b>]')
		banall_user(member_id)
		kick_user(member_id, msg.to.id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, '><i> دسترسی کاربر برای ورود به تمامی گروه های ربات </i> [ @TGtelewhite ] <i> بازگردانده شد </i> \n><i> یوزرنیم: </i> [@'..member..']\n><i> شناسه کاربری: </i> [<b>'..member_id..'</b>]')
